...
class Cache_Driver_File {
	...
	{
		if(isset($config['dir']) === true){
			$this->dir = $config['dir'];
			unset($config['dir']);
		}else{
			$this->dir = Kohana::$cache_dir.'\cachelite\\';
		}
	...
	public function exists($keys, $tag = FALSE)
	{
		...
		elseif ($tag === TRUE)
		{
			...
			{
				$paths = array_merge($paths, glob($this->dir.'*~*'.$tag.'*~*'));
			}
		...
		else
		{
			...
			foreach ( (array) $keys as $key)
			{
				...
				$paths = array_merge($paths, glob($this->dir.str_replace(array('/', '\\', ' '), '_', $key).'~*'));
			}
	...
	public function set($items, $tags = NULL, $lifetime = NULL)
	{
		...
		foreach ($items as $key => $value)
		{
			if (is_resource($value))
				throw new Kohana_Exception('Caching of resources is impossible, because resources cannot be serialised.');

			// Remove old cache file
			$this->delete($key);

			if ( ! (bool) file_put_contents($this->dir.str_replace(array('/', '\\', ' '), '_', $key).'~'.$tags.'~'.$lifetime, serialize($value)))
			{
				$success = FALSE;
			}
		}
	...
	public function get($keys, $single = FALSE)
	{
		...
		// Return a single item if only one key was requested
		if ($single)
		{
	...
	public function delete($keys, $tag = FALSE)
	{
		...
		foreach ($paths as $path)
		{
			...
			if ( ! unlink($path))
			{
				Kohana::$log->add('error', 'Cache: Unable to delete cache file: '.$path);
				$success = FALSE;
			}
		}
